        ...
        <tbody>
          {!!orders?.length &&
            orders?.map((order) => {
              ...
              return !order.order?.every((order) => order.product) ? null : (
                <tr key={order?._id}>
                  ...
                  <td>
                    {order.order.map((order) => (
                      <p key={order?._id} className="table__subFields">
                        ...
                        <b>
                          ({order?.product?.unit ? order?.product?.unit : 'шт.'}
                          )
                        </b>
                      </p>
                    ))}
                  ...
                  <td className="table__xs">
                    {order.userId
                      ? Math.floor(getTotalPrice(order.order[0]))
                      : Math.floor(
                          order.order.reduce(
                            (num, acc) =>
                              num + acc.product.price * acc.quantity,
                            0
                          )
                        )}
                  </td>
                  ...
                  <td className="table__date">
                    ...
                    <a
                      href={
                        'https://electromarket.kg/order-checkout/' + order._id
                      }
